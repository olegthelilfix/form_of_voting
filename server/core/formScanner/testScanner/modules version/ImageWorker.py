...
class ImageWorker:
    ...
    def __init__( self, \
        ...
        if ( status == IMAGE_WORKER ):
            ...
            self.DPI = image.getDPI()

        if ( findEXIF == YES_EXIF ):
            '''
    ...
    def copyOpenCVImageToPIL( self, imageOpenCV ):

        height, width = imageOpenCV.shape[:2]
        size = width, height
        self.imagePIL = Image.fromstring( "L", size, imageOpenCV.tostring())
