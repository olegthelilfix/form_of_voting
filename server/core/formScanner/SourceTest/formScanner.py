...
#БЕРЕМ В КАЧЕСТЕ ЭТАЛОНА РАЗМЕР БОЛЬШОГО КОДА
    height = int(height_big_qr_code / 2)
    ...
    while( 1 ):
        ...
        c = c + 1
        img_with_possible_qr.save( "test2/" + str(c) + fileNameImgWithPossibleQr)
        symbols = scanImageForQRCode( img_with_possible_qr, 0 )
        if len( symbols ):
            ...
            #img_with_possible_qr.save( "test2/" + str( start_y ) + fileNameImgWithPossibleQr )
...
#возвращает контуры найденных МЕТОК
    ...
    for cnt in contours:
        ...
        if ( 0.8 < k and k < 1.8  and cv2.contourArea(cnt) > 1000 ):
            count = count + 1
            print("k = ", k)
...
if ( len( coordinateOfQRCode ) > 0 ):
        ...
        if ( len ( listOfQRCodes ) > 0 ):
                ...
                for i in range( 0, int( len( listOfQRCodes) / 2 ) ):
                        listOfResult = []
                        fileNameImageWithCells = "image_with_cells.png"
                        img = Image.open( fileName )
